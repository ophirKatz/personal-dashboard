  ...
  for (const friend of friends) {
    ...
    if (overdue && !friend.reminder_notified_at) {
      await supabase.from('notifications').insert({
        ...
        message: `You haven't connected in a while — your goal is ${formatFriendGoal(friend.goal_count, friend.goal_unit, friend.goal_mode)}.${friend.notes ? ` Note: ${friend.notes}` : ''}`,
      })
